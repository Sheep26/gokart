...
void Networking::stop_hotspot() {
    std::string cmd = "nmcli connection down Hotspot; nmcli radio wifi on; nmcli connection up \"$(nmcli -t -f NAME connection show | grep -v Hotspot | head -n1)\"";
    FILE* pipe = popen(cmd.c_str(), "r");
